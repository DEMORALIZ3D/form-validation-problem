...
export const validateFormData = (form) => {
    ...
    Object.keys(form).forEach((id) => {
        switch (id) {
            ...
            case 'animals':
                return validated = {...validated, [id]: form[id].value.length === 2};
            case 'typeOfTiger':
                return validated = {...validated, [id]: !form['animals'].value.includes('tiger') ? true : NOT_EMPTY_REGEX.test(form[id].value)};
            default:
                break;
        }
    });
